...
const TABLES = ['actions', 'decisions', 'risks', 'milestones', 'people', 'meetings', 'budget', 'grants',
  'reviews', 'incidents', 'policies', 'contacts', 'teams', 'docs', 'members'];
...
async function loadAll() {
  ...
  const { data: auditRows } = await sb.from('audit').select('*').order('t', { ascending: false }).limit(100);
  S.audit = auditRows || [];
}
...
function loadingHtml() {
  return `<div class="setup">${star('bigstar')}
    <div class="setup-card" style="text-align:center"><div class="display" style="color:var(--purple)">Loading the portal…</div></div></div>`;
}
...
function renderAuthScreen(sentTo) {
  ...
  document.getElementById('si-go').addEventListener('click', async () => {
    ...
    if (error) { alert('Could not send link: ' + error.message); btn.disabled = false; btn.textContent = 'Send sign-in link'; return; }
...
async function boot(newSession) {
  ...
  if (!member || !member.active) { CM = null; renderNotInvited(); return; }
  CM = member;
...
const TAB_OF = { home: 'home', year: 'year', meetings: 'meetings', meeting: 'meetings', registers: 'registers', more: 'more', compliance: 'more', finance: 'more', review: 'more', docs: 'more', incidents: 'more', contacts: 'more', handover: 'more', membership: 'more', activity: 'more', members: 'more' };
...
function membersPage() {
  ...
  out += `<div class="rowlist">` + list.map((m) => `
    <div class="card row" data-modal="member:${m.id}" style="cursor:pointer;opacity:${m.active ? 1 : 0.5}">
      ...
      <div class="m">${esc(m.role)} · ${esc(m.email)}</div>
      <div style="display:flex;gap:5px;margin-top:7px;flex-wrap:wrap">
  ...
  if (!S.docs.length) {
    ...
    out += `<div class="rowlist">` + list.map((d) => `
      <div class="card row docrow" data-modal="doc:${d.id}" data-search="${esc((d.name + ' ' + d.cat + ' ' + (d.notes || '')).toLowerCase())}" style="cursor:pointer">
        ...
        <div class="m">${esc(d.cat)}${d.where ? ` · lives in: ${esc(d.where)}` : ''}${d.notes ? ` · ${esc(d.notes)}` : ''}</div></div>
        ${d.file_path ? `<button class="btn btn-ghost btn-sm" data-open-doc="${d.id}" onclick="event.stopPropagation()">Open</button>` : `<span class="tag tag-grey">No file</span>`}
      </div>`).join('') + `</div>`;
  }

  out += `<div class="docnote" style="margin-top:16px"><b>Policies note:</b> review dates for policies are tracked
    in Compliance & Policies. When uploads go live, a new version of a policy will keep the old one as history
    and prompt the review date to update.</div>`;
  return out;
}
...
function modalHtml() {
  ...
  if (kind === 'action') {
    ...
    body = `<textarea id="f-export" style="min-height:240px;font-size:12px" readonly>${esc(JSON.stringify({ exportedAt: new Date().toISOString(), actions: S.actions, decisions: S.decisions, risks: S.risks, milestones: S.milestones, people: S.people, clubComp: S.clubComp, meetings: S.meetings, budget: S.budget, grants: S.grants, reviews: S.reviews, incidents: S.incidents, policies: S.policies, contacts: S.contacts, handover: S.handover, teams: S.teams, docs: S.docs }, null, 2))}</textarea>
    ...
    const dd = isNew ? { cat: 'Policies' } : S.docs.find((x) => x.id === id) || {};
    ...
    body = field('Document', inp('f-title', dd.name, 'e.g. Safeguarding Policy')) +
      ...
      (dd.file_path ? `<div class="docnote" style="margin-bottom:13px">A file is attached. Choosing a new one replaces it.</div>` : '') +
      field('Where else it lives (optional)', inp('f-where', dd.where, 'e.g. Google Drive > Club folder')) +
      field('Notes', txt('f-notes', dd.notes));
  } else if (kind === 'member') {
    const m = isNew ? { role: 'Committee Member', is_admin: false, can_safeguarding: false, active: true } : S.members.find((x) => x.id === id) || {};
    ...
    body = field('Name', inp('f-name', m.name, 'e.g. Sam Taylor')) +
      ...
      (isNew ? `<div class="docnote">A sign-in link is emailed the first time this person visits the portal and enters their email — there\u2019s no password to set here.</div>` : '');
  ...
  return `<div class="shell">
    ...
    <div>
      <header class="topbar">
        ...
        <div class="who"><b>${esc(CM.name)}</b>${esc(CM.role)}</div>
      </header>
      <main>${content}</main>
    </div>
    ${modalHtml()}
...
function render() {
  ...
  const pages = {
    ...
    activity: activityPage, members: membersPage,
  };
  document.getElementById('app').innerHTML = shellHtml((pages[S.page] || dashboardPage)());
  bindRiskSliders();
  bindDocSearch();
  document.getElementById('signout-link')?.addEventListener('click', async (e) => { e.preventDefault(); await sb.auth.signOut(); });
}
...
const TABLE_OF_KIND = {
  ...
  contact: 'contacts', team: 'teams', doc: 'docs', member: 'members',
};
const labelFor = (obj) => obj.title || obj.name || obj.season || (obj.summary || '').slice(0, 40) || 'item';

async function uploadDocFile(docId, cat) {
  const fileInput = document.getElementById('f-file');
  const file = fileInput && fileInput.files && fileInput.files[0];
  if (!file) return null;
  const path = `${(cat || 'Other').replace(/\W+/g, '-')}/${docId}-${file.name}`;
...
async function saveModal(kind, id) {
  ...
  if (kind === 'action') {
    ...
    if (!val('f-title').trim()) return;
    payload = { title: val('f-title').trim(), date: val('f-date') || null, proposer: val('f-proposer').trim(), meeting: val('f-meeting').trim(), notes: val('f-notes') };
  } else if (kind === 'risk') {
    if (!val('f-title').trim()) return;
    ...
    if (!val('f-title').trim()) return;
    payload = { title: val('f-title').trim(), date: val('f-date'), cat: val('f-cat'), notes: val('f-notes') };
    if (isNew) payload.prep = [];
  } else if (kind === 'person') {
    ...
    if (!val('f-name').trim() || !val('f-email').trim()) return;
    payload = { name: val('f-name').trim(), email: val('f-email').trim().toLowerCase(), role: val('f-role'), is_admin: checkedBox('f-admin'), can_safeguarding: checkedBox('f-safeguarding') };
    payload.active = isNew ? true : checkedBox('f-active');
  }
  ...
  if (kind === 'doc') {
    ...
    if (path) {
      const { data: updated } = await sb.from('docs').update({ file_path: path }).eq('id', row.id).select().single();
  ...
  S.modal = null; S.prefill = null; render();
}

async function voidItem(kind, id) {
...
function doExport() {
  const payload = { exportedAt: new Date().toISOString(), actions: S.actions, decisions: S.decisions, risks: S.risks,
    milestones: S.milestones, people: S.people, clubComp: S.clubComp, meetings: S.meetings, budget: S.budget,
    grants: S.grants, reviews: S.reviews, incidents: S.incidents, policies: S.policies, contacts: S.contacts,
    handover: S.handover, teams: S.teams, docs: S.docs };
  try {
...
document.addEventListener('click', (e) => {
  ...
  const t = e.target.closest('[data-go],[data-modal],[data-filter],[data-reg],[data-toggle],[data-ms-toggle],'
    + '[data-save],[data-del],[data-void],[data-close],[data-open-meeting],[data-ag-add],[data-ag-del],'
  ...
  if (d.go) { S.page = d.go; render(); }
    ...
    if (input && input.value.trim()) {
      ...
      if (m) updateMeeting(d.agAdd, { agenda: [...m.agenda, { id: uid(), title: input.value.trim(), notes: '' }] }).then(render);
    }
  }
  else if (d.agDel) {
