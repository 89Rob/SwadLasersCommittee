...
sb.auth.onAuthStateChange((event, s) => {
  // Any auth event that just reasserts the SAME already-signed-in person —
  // a background token refresh, a visibility/focus-triggered session check
  // when the tab regains focus after the file picker or screen lock, etc. —
  // shouldn't blow away the running app and any open modal. Rather than
  // trying to guess which event names Supabase might use for this (it isn't
  // consistent), just check whether the person is unchanged. Only a genuine
  // change of who's signed in (including signing out) or the very first
  // boot (CM not set yet) should trigger a full reboot.
  const sameUser = CM && s && s.user && s.user.email && s.user.email.toLowerCase() === CM.email.toLowerCase();
  if (sameUser) {
    session = s;
    return;
  }
...
function meetingPage() {
  ...
  // Matches meetings_update exactly: admins can always edit; everyone else
  // only while the meeting hasn't happened yet. Once it's in the past, the
  // database itself would reject the save — so don't show editable controls
  // that are guaranteed to fail for non-admins.
  const canEditMeeting = CM.is_admin || m.date >= today();
  ...
  out += `<div class="card" style="margin-bottom:14px">
    ...
    ${canEditMeeting
      ? `<input data-set="attendees:${m.id}" value="${esc(m.attendees)}" placeholder="Who was there?">`
      : `<div class="m">${esc(m.attendees) || 'Not recorded'}</div>`}</div></div>`;
  ...
  out += m.agenda.map((item, i) => `
    <div class="card">
      <div class="row" style="margin-bottom:8px">
        ...
        ${canEditMeeting ? `<div style="display:flex;gap:6px;flex:none">
      ...
      ${canEditMeeting
        ...
        : (item.notes ? `<div class="m" style="white-space:pre-wrap">${esc(item.notes)}</div>` : '')}
    </div>`).join('');
  out += `</div>`;
  if (!canEditMeeting) {
    out += `<div class="docnote" style="margin-top:10px">This meeting has passed \u2014 minutes are locked. Only an admin can amend them now.</div>`;
...
// Modals
// ============================================================
// These mirror the real database UPDATE/DELETE policies exactly — not a
// separate guess at permissions, but the same boundaries already enforced
// server-side. A control is only worth showing if the save behind it would
// actually be allowed; otherwise it's just a guaranteed "no permission" toast.
function canWriteKind(kind) {
  switch (kind) {
    case 'decision': return CM.is_admin;
    case 'incident': return CM.is_admin || CM.can_safeguarding;
    case 'budget':
    case 'grant': return CM.is_admin || CM.can_finance;
    case 'member': return CM.is_admin;
    default: return true;
  }
}
function canDeleteKind(kind) {
  switch (kind) {
    case 'doc': return CM.is_admin;
    case 'budget':
    case 'grant': return CM.is_admin || CM.can_finance;
    case 'member': return CM.is_admin;
    default: return true;
...
function modalHtml() {
  ...
  const canWrite = isNew || canWriteKind(kind);
  return `<div class="overlay" id="overlay"><div class="sheet" role="dialog" aria-label="${title}">
    <h2 class="display">${title}</h2>${body}
    <div class="actions">
      ${isNew || kind === 'clubcomp' ? '' : VOID_KINDS.includes(kind)
        ? (isVoided || !canWrite ? '' : `<button class="btn btn-danger" data-voidask="${kind}:${id}">Mark void</button>`)
        : (canDeleteKind(kind) ? `<button class="btn btn-danger" data-del="${kind}:${id}">Delete</button>` : '')}
      ${extraBtns}
      ${canWrite
        ? `<button class="btn btn-ghost" data-close="1">Cancel</button><button class="btn btn-gold" data-save="${kind}:${id}">Save</button>`
        : `<button class="btn btn-gold" data-close="1" style="flex:1">Close</button>`}
    </div></div></div>`;
}
// ============================================================
// Navigation & shell
...
async function deleteItem(kind, id) {
  ...
  // Deleting a document record should also remove its actual file(s) from
  // storage — including any prior versions — otherwise every deletion leaves
  // an orphaned file behind that nothing points to or ever cleans up again.
  if (kind === 'doc') {
    const paths = [item.file_path, ...(Array.isArray(item.versions) ? item.versions.map((v) => v.file_path) : [])].filter(Boolean);
    if (paths.length) {
