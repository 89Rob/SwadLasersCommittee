...
sb.auth.onAuthStateChange((event, s) => {
  // A routine background token refresh shouldn't nuke the whole app — that
  // wipes any open modal and whatever's mid-typing. This fires exactly when
  // picking a file backgrounds the tab and the session silently refreshes
  // the moment focus returns. Only a genuine sign-in/out (or the very first
  // load, when CM isn't set yet) should trigger a full reboot.
  if (event === 'TOKEN_REFRESHED' && CM) {
    session = s;
    return;
  }
  boot(s);
