...
function loadState() {
  const base = {
    ...
    incidents: [], policies: seedPolicies(), contacts: [], handover: {}, teams: [], audit: [], docs: [],
  };
  try {
...
const S = Object.assign(loadState(), { page: 'home', filter: 'open', reg: 'actions', modal: null, meetingId: null, prefill: null, docCat: 'All' });
S.docs = (S.docs || []).map((d) => OLD_CATS[d.cat] ? { ...d, cat: OLD_CATS[d.cat] } : d);
...
function saveState() {
  mem = {
    ...
    incidents: S.incidents, policies: S.policies, contacts: S.contacts, handover: S.handover, teams: S.teams, audit: S.audit, docs: S.docs,
  };
...
function morePage() {
  const items = [
    ...
    ['docs', ICONS.docs, 'Documents', 'The document library — indexed now, uploads with sign-in'],
  ];
...
function docsPage() {
  let out = pageHead('Library', 'Documents', 'The club\u2019s papers — indexed now, uploaded here once sign-in is live.',
    `<div style="display:flex;gap:8px;flex-wrap:wrap">
      <button class="btn btn-ghost" data-modal="upload:info">⬆ Upload</button>
      <button class="btn btn-gold" data-modal="doc:new">+ Add record</button>
    </div>`);
  ...
  if (!S.docs.length) {
    out += `<div class="card empty"><div class="display">No documents indexed yet</div>
      <p>Add a record for each club document and where it currently lives (Drive, email, someone\u2019s laptop…).
      When sign-in arrives, the files themselves move in here.</p>
      <button class="btn btn-gold" data-modal="doc:new">Index the first document</button></div>`;
  } else if (!list.length) {
...
function modalHtml() {
  ...
  if (kind === 'action') {
    ...
    title = isNew ? 'Add document record' : 'Document record';
    body = field('Document', inp('f-title', dd.name, 'e.g. Safeguarding Policy')) +
      ...
      field('Where it currently lives', inp('f-where', dd.where, 'e.g. Google Drive > Club folder, or Secretary\u2019s laptop')) +
      field('Notes', txt('f-notes', dd.notes));
  } else if (kind === 'upload') {
    title = 'Uploads arrive with sign-in';
    body = `<div class="docnote" style="margin-bottom:13px">
      Nearly there — file uploads need shared, secure storage so a document one person uploads is safely
      visible to the whole committee. That comes with the Supabase upgrade (committee sign-in), which is the
      next build step.<br><br>
      Until then, use <b>+ Add record</b> to index each document and where it lives, so the library is
      already organised the day uploads switch on.</div>`;
    return `<div class="overlay" id="overlay"><div class="sheet" role="dialog" aria-label="${title}">
      <h2 class="display">${title}</h2>${body}
      <div class="actions"><button class="btn btn-gold" data-close="1" style="flex:1">Got it</button></div></div></div>`;
...
function saveModal(kind, id) {
  ...
  if (kind === 'action') {
    ...
    if (!val('f-title').trim()) return;
    S.docs = upsert(S.docs, id, { name: val('f-title').trim(), cat: val('f-cat'), version: val('f-version').trim(), where: val('f-where').trim(), notes: val('f-notes') });
  }
...
function deleteItem(kind, id) {
  ...
  const map = { action: 'actions', decision: 'decisions', risk: 'risks', milestone: 'milestones', person: 'people', budget: 'budget', grant: 'grants', review: 'reviews', meeting: 'meetings', incident: 'incidents', policy: 'policies', contact: 'contacts', team: 'teams', doc: 'docs' };
...
document.addEventListener('click', (e) => {
  ...
  const t = e.target.closest('[data-go],[data-modal],[data-filter],[data-reg],[data-toggle],[data-ms-toggle],[data-save],[data-del],[data-void],[data-close],[data-open-meeting],[data-ag-add],[data-ag-del],[data-quick-action],[data-quick-decision],[data-minutes],[data-prep],[data-export],[data-doccat]');
  if (!t) return;
  const d = t.dataset;
