...
const TAB_OF = { home: 'home', year: 'year', meetings: 'meetings', meeting: 'meetings', registers: 'registers', more: 'more', compliance: 'more', finance: 'more', review: 'more', docs: 'more', incidents: 'more', contacts: 'more', handover: 'more', membership: 'more', activity: 'more', members: 'more', search: 'more' };
...
// ============================================================
// ============================================================
// Page: Search — across actions, decisions and meeting minutes
// ============================================================
function searchPage() {
  let out = pageHead('Find', 'Search', 'Across actions, decisions and every meeting\u2019s minutes.');
...
function runGlobalSearch(q) {
  ...
  if (!query) { results.innerHTML = `<div class="card empty"><p style="margin:0">Start typing to search actions, decisions and minutes.</p></div>`; return; }

  const hits = [];
  S.actions.forEach((a) => {
    if ((a.title + ' ' + (a.notes || '')).toLowerCase().includes(query)) {
      hits.push({ type: 'Action', title: a.title, meta: `${a.owner || 'Unassigned'} · due ${fmtDate(a.due)}`, go: 'registers', pre: () => { S.reg = 'actions'; } });
    }
  });
  S.decisions.forEach((d) => {
    if ((d.title + ' ' + (d.notes || '')).toLowerCase().includes(query)) {
      hits.push({ type: 'Decision', title: d.title, meta: fmtDate(d.date), go: 'registers', pre: () => { S.reg = 'decisions'; } });
    }
  });
  ...
  results.innerHTML = `<div class="rowlist">` + hits.map((h) => `
    <div class="card row" data-search-go="${h.go}" data-search-meeting="${h.meetingId || ''}" data-search-reg="${h.pre ? (h.go === 'registers' ? '1' : '') : ''}" style="cursor:pointer">
      <div class="body"><div class="t">${esc(h.title)}</div><div class="m">${esc(h.meta)}</div></div>
      <span class="tag tag-open">${h.type}</span>
    </div>`).join('') + `</div>`;

  // Store the pre-navigation callbacks against their rows without re-rendering the whole app
  results.querySelectorAll('[data-search-go]').forEach((el, i) => {
    el.addEventListener('click', () => {
      if (hits[i].pre) hits[i].pre();
      if (hits[i].meetingId) S.meetingId = hits[i].meetingId;
      S.page = hits[i].go;
      render();
    });
  });
...
function membersPage() {
  ...
  out += `<div class="rowlist">` + list.map((m) => `
    <div class="card row" data-modal="member:${m.id}" style="cursor:pointer;opacity:${m.active ? 1 : 0.5}">
      ...
      ${!m.active ? `<button class="btn btn-ghost btn-sm" data-offboard="${m.id}" onclick="event.stopPropagation()">${m.offboarding ? 'Offboarding \u2713' : 'Offboarding'}</button>` : ''}
    </div>`).join('') + `</div>`;
...
function modalHtml() {
  ...
  if (S.modal.startsWith('voidconfirm:')) {
    ...
    const labels = { decision: 'decision', incident: 'incident' };
...
function render() {
  ...
  const pages = {
    ...
    activity: activityPage, members: membersPage, search: searchPage,
  };
...
async function saveModal(kind, id) {
  ...
  if (kind === 'action') {
    ...
    if (isNew) {
      ...
      if (data && data.error) { alert('Could not invite: ' + data.error); return; }
...
document.addEventListener('click', (e) => {
  ...
  const t = e.target.closest('[data-go],[data-modal],[data-filter],[data-reg],[data-toggle],[data-ms-toggle],'
    + '[data-save],[data-del],[data-voidask],[data-voidconfirm],[data-close],[data-open-meeting],[data-ag-add],[data-ag-del],'
    + '[data-quick-action],[data-quick-decision],[data-minutes],[data-prep],[data-export],[data-doccat],[data-open-doc],'
    + '[data-open-docpath],[data-print-minutes],[data-ical-meeting],[data-ical-milestone],[data-offboard],[data-save-offboard]');
  if (!t) return;
  const d = t.dataset;
  ...
  else if (d.saveOffboard) { saveOffboarding(d.saveOffboard); }
  else if (d.agAdd) {
